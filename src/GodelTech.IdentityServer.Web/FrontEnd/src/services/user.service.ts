...
import { EventHistoryData } from "@models/event-history-data.model";
import { ResetPassword } from "@models/reset-password.model";
import { ListOf, ListOfUsers, UserProfile } from "@models/user-management-api.models";
import { AdminAddNewUser } from "@models/admin-add-new-user.model";
import { Claim } from "@models/claim.model";
import { UserLogin } from "@models/user-login.model";
import { Role, SaveRole } from "@models/role.model";
...
export class UserService {
    ...
    public findUsers(text: string, quantity: number, page: number): Observable<ListOfUsers> {
        return this.http.get<ListOfUsers>(
            `${this.endpoint}?limit=${quantity}&offset=${(page - 1) * quantity}&search=${encodeURI(text)}`,
        );
    }
    ...
    public removeLogin(userName: string, loginProvider: string, providerKey: string): any {
        return this.http.delete<void>(
            `${this.endpoint}/${userName}/logins?loginProvider=${loginProvider}&providerKey=${providerKey}`,
        );
    }
    ...
    public showEvents(username: string, quantity: number, page: number): Observable<ListOf<EventHistoryData>> {
        return this.http.get<ListOf<EventHistoryData>>(
            `${this.endpoint}/${username}/logs?limit=${quantity}&offset=${(page - 1) * quantity}`,
        );
    }
    ...
    public searchEvents(
        ...
        quantity: number,
        page: number,
    ): Observable<ListOf<EventHistoryData>> {
        return this.http.get<ListOf<EventHistoryData>>(
            `${this.endpoint}/${username}/logs?limit=${quantity}&offset=${(page - 1) * quantity}&search=${encodeURI(
                text,
            )}`,
        );
    }
